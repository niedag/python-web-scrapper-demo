...
def scrape_url(url, config):
    ...
    title = title_element['content'] if title_element and title_element.has_attr('content') else title_element.get_text(strip=True) if title_element else "Title not found"

    description_element = soup.select_one(config['description_selector'])
    description = description_element['content'] if description_element and description_element.has_attr('content') else description_element.get_text(strip=True) if description_element else "Description not found"

    decompose_embeds(soup, config.get('embed_selectors', []))
    body_text = process_body(soup, config['body_selector'], config['paragraph_selector'])
